...
export default function LoginPage() {
  const { t, language, profileIndex, setProfileIndex, login } = useApp();
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (!otpStage) {
        ...
        if (ok) {
          // Sign in as the worker whose credentials were used, so Home reflects
          // who logged in rather than a pre-picked demo profile.
          const match = profiles.findIndex((p) => p.phone === phone.trim() || p.jobCard === jobCard.trim());
          login(match >= 0 ? match : profileIndex);
          router.push("/home");
  ...
  return (
    <main className="login-page">
      <BrandHeader minimal onHome={onBack} onBack={onBack} />
      ...
      <div className="login-body">
        <section className="login-hero">
          ...
          <p className="synthetic-note login-hero-note"><span className="note-dot" aria-hidden="true" />{t.synthetic}</p>
        </section>
